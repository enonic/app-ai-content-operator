...
export default function AssistantIcon({ className, animated = false }: Props): React.ReactNode {
  const pupilId = useId();
  const eyeId = useId();
  ...
  return (
    <div
      className={twMerge(
        'flex justify-items-center items-center w-8 h-8 rounded-full bg-white',
        className,
      )}
    >
      <svg
