...
import { getNextActiveNode, pruneGraph } from '@/common/graph';
...
import { $history, $lastMessage, $messages, $startId } from './chat.store';
...
function addChatMessage<T extends ChatMessage>(message: T): Optional<T> {
  ...
  const currentActiveMessage =
    message.active && forMessage && getNextActiveNode($messages.get(), forMessage.id);
...
): Readonly<Optional<UserChatMessage>> {
...
export function addModelMessage(
  analysisResult: AnalysisResult,
  forId?: string,
): Readonly<Optional<ModelChatMessage>> {
  return addChatMessage({
    id: nanoid(),
...
): Readonly<Optional<SystemChatMessage>> {
...
export function updateModelMessage(
  ...
  return updateChatMessage({
    ...
    content: {
      ...message.content,
      generationResult: generationResult ?? message.content.generationResult,
    },
  } satisfies ModelChatMessage);
}
...
function replaceWithSystemMessage(
  message: string,
...
export function addErrorMessage(
  payload: FailedMessagePayload | string,
  messageToReplaceId?: string,
): void {
...
export function addStoppedMessage(
  role: Exclude<MessageRole, 'model'>,
  messageToReplaceId?: string,
): void {
  const message =
    role === MessageRole.USER ? t('text.system.stopped.user') : t('text.system.stopped.system');
