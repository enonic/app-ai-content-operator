...
function createRequestOptions({
  ...
  body,
  queryParams,
}: RequestParams): HttpClientRequestOptions {
  return {
    url,
    method,
    headers: {
...
function parseHttpLibError(error: unknown): AiError {
  ...
  if (
    error.message.indexOf("couldn't receive headers on time") >= 0 ||
    error.message.indexOf('closed') >= 0
  ) {
