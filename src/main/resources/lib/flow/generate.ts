...
function isAnalysisObjectEntry(
  entry: AnalysisObjectEntry | AnalysisReferenceEntry,
): entry is AnalysisObjectEntry {
...
export function fixResultFields(
...
export function attemptResultNormalization(
  result: Record<string, unknown>,
): Record<string, unknown> {
  const fixedResult: Record<string, unknown> = {};
